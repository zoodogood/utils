...
class GlitchText {
	...
	constructor(
		from = '',
		to = 'hello, world',
		{ step = 15, random = false, maximum = 100 }: IGlitchTextOptions = {},
	) {
		this.from = from
		this.to = to
	...
	*iteratorFunction() {
		...
		while (word.length !== target.length) {
			if (word.length > target.length) word.pop()
		...
		while (word.join('') !== target) {
			word.forEach((letter, i) => {
				...
				if (this.random) step *= Math.random() * 2

				word[i] = String.fromCharCode(
					isUpper ? charCode + step : charCode - step,
				)
			})
