...
var concatAssign: <
	...
	A extends
		...
		| (B extends unknown[] ? { push: (this: A, ...b: B) => void } : never),
>(
	lhs: A,
	rhs: B,
) => A = (lhs, rhs) => (
	((rhs as any)?.[Symbol.isConcatSpreadable] ?? Array.isArray(rhs))
...
export enum ArrayEmulateStrategy {
	Partial,
	Proxied,
}
...
type LazySortInner<T> = Configuration & {
	chunks: Chunk<T>[]
	min: number
	max: number
	calculated: boolean
...
export class LazySort<T> {
	...
	static ofNumbers(numbers: number[]) {
		return new LazySort(numbers.map(($1) => [$1, $1]))
	}
	static ofArrayMapValue<T>(
		array: T[],
		itemOf: (T: T, index: number, arr: T[]) => T,
	) {
		return new LazySort(
			array.map(($, i, arr) => [$, itemOf($, i, arr)] as WithValue<T>),
		)
	}

	constructor(
		iterable1: WithValue<T>[],
		{
			revert: revert1 = false,
			chunks_count: chunks_count1 = GOOD_CHUNKS_COUNT,
			emulate: emulate1 = ArrayEmulateStrategy.Partial,
		}: Configuration = {},
	) {
	...
	chunks_update_thresholds_after(chunk_index: number, diff: number) {
		const results = []
		for (
			let end1 = this.chunk_thresholds!.length, i1 = chunk_index;
			i1 < end1;
			++i1
		) {
			const index = i1
	...
	entry() {
		if (!this.calculated) {
			...
			this.min = min1
			const chunk_range1 = Math.ceil(
				(this.max! - this.min!) / (this.chunks_count - 1),
			)
			this.chunk_range = chunk_range1
			this.is_atomic =
				this.chunks_count >= this.length || this.max - this.min === 0
			const chunks1 = (() => {
				if (!this.is_atomic) {
					...
					this.iterable.forEach((x: WithValue<T>) => {
						return chunks[
							Math.floor((x[1] - this.min!) / this.chunk_range!)
						].push(x)
					})
					return chunks.map(
						($4) =>
					...
					return [
						new LazySort<T>(
							this.iterable.toSorted(
								(a, b) => (-1) ** +this.revert * (a[1] - b[1]),
							),
							{
								revert: this.revert,
								chunks_count: this.chunks_count,
			...
			const chunk_thresholds1 = this.chunks?.reduce(
				(acc: number[], next) =>
					acc.concat([(acc[acc.length - 1] || 0) + next.length]),
				[],
			)
		...
		switch (this.emulate) {
			case ArrayEmulateStrategy.Partial: {
				return this.#to_partial_array_emulator()
			}
			case ArrayEmulateStrategy.Proxied: {
				return this.#to_proxied_array_emulator()
			}
		}
	}
}
...
export class LazySortEntry<T> extends Array {
	...
	*[Symbol.iterator](): Generator<WithValue<T>> {
		for (let end2 = this.#source.length, i2 = 0; i2 < end2; ++i2) {
			const index = i2
			yield this.positive_at(index)!
		}
	}
	toArray(): WithValue<T>[] {
		if (this.#source.is_atomic) {
			return this.#source.atomic_chunk_arr
		}
		return this.#source.chunks!.flatMap(($5) => $5.entry().toArray())
	}
	at(at: number) {
		return at < 0
			? this.positive_at(this.#source.length - at)
			: this.positive_at(at)
	}
	positive_at(at: number): WithValue<T> | undefined {
		if (this.#source.is_atomic!) {
			return this.#source.atomic_chunk_arr[at]
		}
		const chunk_index = this.#source.chunk_thresholds!.findIndex(
			(a1) => a1 > at,
		)
		if (chunk_index === -1) {
			return undefined
		...
		return chunk
			.entry()
			.positive_at(at - (this.#source.chunk_thresholds![chunk_index - 1] || 0))
	}
	...
	findIndex(predicate: Predicate<WithValue<T>>) {
		for (let end3 = this.length, i3 = 0; i3 < end3; ++i3) {
			...
			if (
				predicate(
					this.positive_at(index)!,
					index,
					this as unknown as WithValue<T>[],
				)
			) {
				return index
			}
	...
	indexOf(item: WithValue<T>): number {
		if (this.#source.is_atomic!) {
			return this.#source.atomic_chunk_arr.indexOf(item)
		}
		let results1
		results1 = []
		let ref1
		for (const chunk_index in (ref1 = this.#source.chunks!)) {
	...
	splice(
		...
		start + deleteCount > this.#source.length &&
			(deleteCount = this.#source.length - start)
		deleteCount - items.length !== 0 &&
			(this.#source.length -= deleteCount - items.length)
		if (this.#source.is_atomic!) {
			return this.#source.atomic_chunk_arr.splice(start, deleteCount, ...items)
		}
		...
		this.#source
			...
			.forEach(
				({ chunk_start, chunk_index, chunk }, changed_index, changes_array) => {
					const replace_start = changed_index === 0 ? start - chunk_start : 0

					const replace_end =
						changed_index === changes_array.length - 1
							? end - chunk_start
							: chunk.length

					const _removed_count = replace_end - replace_start
					const _added_items = items.slice(
						replace_start + chunk_start - start,
						changed_index === changes_array.length - 1
							? undefined
							: replace_end + chunk_start - start,
					)
					this.#source.chunks_update_thresholds_after(
						chunk_index,
						_added_items.length - _removed_count,
					)
					removed.push(
						...chunk
							.entry()
							.splice(replace_start, _removed_count, ..._added_items),
					)
				},
			)
