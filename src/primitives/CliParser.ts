...
export class FlagsManager {
	...
	captureResidueFlags(context: CliParserRunContext) {
		...
		while (true) {
			const match = parser.replaceByMatch(
...
export class CapturedContent {
	...
	static regexArrayOrStringToString(content: TCaptureValue): string {
		if (CapturedContent.contentIsString(content)) {
	...
	static contentIsRegExpMatchArray(content: TCaptureValue): boolean {
		return (
			...
			Object.hasOwn(content, 'index') &&
			Object.hasOwn(content, 'input')
		)
	}
	...
	static contentIsBracketGroupStamp(
		...
		content = CapturedContent.regexArrayOrStringToString(content)
	...
	static contentToString(
		...
		let value = ((content: TCaptureValue) => {
			...
			if (CapturedContent.contentIsRegExpMatchArray(content)) {
				content = CapturedContent.regexArrayOrStringToString(content)
			}
			if (
				context &&
				CapturedContent.contentIsBracketGroupStamp(content, context)
			) {
				return (
					CapturedContent.contentToGroupElement(content, context)?.content ||
					content
				)
			}
			return content
