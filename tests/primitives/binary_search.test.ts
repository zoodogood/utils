...
import {
	type BinarySearchCompareSpec,
	binary_search,
} from '../../src/primitives/binary_search'

test('binary_search', () => {
	const target = 3
	const comparator = vi.fn((value: BinarySearchCompareSpec) => value - target)
	const index = binary_search(1_000_000, comparator)
	expect(comparator).toBeCalledTimes(20)
